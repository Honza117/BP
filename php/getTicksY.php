...
if ($from_length < $where_length){
    $qry = "SELECT vzdalenost FROM stanice WHERE trat_id='$from_id' AND vzdalenost BETWEEN $from_length AND $where_length ORDER BY vzdalenost"; //od nejmensi po nejvetsi
}
else {
    $qry = "SELECT vzdalenost FROM stanice WHERE trat_id='$from_id' AND vzdalenost BETWEEN $where_length AND $from_length ORDER BY vzdalenost DESC"; //od nejvetsi po nejmensi
}
