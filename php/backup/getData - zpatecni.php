...
foreach ($stations as $key => $value){
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            if (($row["typ"] == "os")&&(!in_array(intval($row["vlak_id"]), $blue_trains_id))){
                $blue_trains_id[] = intval($row["vlak_id"]); //Pole os vlaku pro barvu (modrou)
            }

            //Pokud smer s, ulozim id vlaku se smerem j
            if (((!$desc && $both) && (intval($row["smer_id"]) == 2) && (!in_array(intval($row["vlak_id"]), $trains_j)))){
                $trains_j[] = intval($row["vlak_id"]);
            }

            //Pokud smer j, ulozim id vlaku se smerem s
            if ((($desc && $both) && (intval($row["smer_id"]) == 1) && (!in_array(intval($row["vlak_id"]), $trains_s)))){
...
foreach ($trains as $key => $value){
    ...
    //$json_data["p"]["role"] = "tooltip"; //lze psát info
}   
...
//print_r($trains);
//echo "<br><br>";
$os_trains = array(); //Pole obsahujici poradu spoju urcene $connect_cnt pro barvu (modrou)
    ...
    foreach ($train_inf as $conn_num => $con_arr){
        if (is_numeric($conn_num)){ //abych nevybral [cislo]
            ...
            foreach ($con_arr as $key => $value){
                ...
                //Pokud vykresluju i zpatecni
                if (in_array($train_id, $trains_j)){
                    $submember_data[$vzdalenost]["prijezd"][] = $dep;
                    $submember_data[$vzdalenost]["odjezd"][] = $arr;
                }
                else if (in_array($train_id, $trains_s)){
                    $submember_data[$vzdalenost]["prijezd"][] = $dep;
                    $submember_data[$vzdalenost]["odjezd"][] = $arr;
                }
                else{
    ...
    foreach ($times as $train_num => $time){
        ...
        foreach ($time as $key => $value){
            ...
            if ($value == "null"){
                ...
                $color_cnt = 0; //Pocitadlo pro urceni barvy spoje

                for ($i = 1; $i <= $connect_cnt*2; $i++){
                    $last_i++;
                    $color_cnt++; 
                    ...
                    /*else if (($time[$key] == $time[$i-$last_i])&&($i > 1)){
                        $row_submember["v"] = $station;
                        $row_member["c"][$i] = $row_submember;
                    }*/
                    else {
                    ...
                    //Pokud se spracovava OS vlak z $os_trains, je modry, jinak cerveny
                    if (in_array($color_cnt, $os_trains)){
                        $i++;
                        //echo "modry: " . $i . "<br>";
